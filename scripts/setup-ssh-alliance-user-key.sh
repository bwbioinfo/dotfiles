...
SSH config block. By default this updates the chezmoi source file:

  private_dot_ssh/private_config.d/private_alliance.conf

The script writes only SSH config text. It does not read, copy, create, move,
or modify SSH key files.
EOF
}
...
default_config="$repo_dir/private_dot_ssh/private_config.d/private_alliance.conf"
